...
def select_main_api_from_pages(all_pages):
    ...
    for p in all_pages:
        ...
        weight = 0.5 if API_BAD_CTX.search(tx or "") else 1.0
        for a in apis:
            cnt[a] += weight
            if a not in first_page or pno < first_page[a]:
                first_page[a] = pno
...
def load_auto_api_jsonl(path: Path):
    ...
    with path.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            if rel: mp[str(rel)] = api
            if src: mp[str(Path(src).name)] = api
            if pid: mp[str(pid)] = api
    return mp
...
def resolve_api_with_overrides(current_api, rel_path, src_pdf, jf_name, auto_map, header_map, all_pages):
    ...
    for k in keys:
        if auto_map and k in auto_map:
            return auto_map[k]
    for k in keys:
        if header_map and k in header_map:
            return header_map[k]

    return select_main_api_from_pages(all_pages)
...
def main():
    ...
    auto_map = load_auto_api_jsonl(Path(args.auto_api_jsonl).expanduser().resolve()) if args.auto_api_jsonl else {}
    header_map = load_header_overrides_json(Path(args.header_api_overrides_json).expanduser().resolve()) if args.header_api_overrides_json else {}
    ...
    report = dict(
        ...
        stats=dict(
            wells_total=0,
            api_present=0,
            api_filled_by_auto=0,
            api_filled_by_header_override=0,
            api_filled_by_fulltext_vote=0,
            latlon_present=0,
            latlon_suspect=0,
            stim_present=0,
            stim_has_fields=0,
        ...
        try:
            ...
            if api_after:
                ...
                if not api_before:
                    ...
                    if src_pdf: keys.append(str(Path(src_pdf).name))
                    keys.append(jf.name)
                    if jf.name.lower().endswith(".json"):
                        keys.append(jf.name[:-5] + ".pdf")

                    if any(k in auto_map for k in keys):
                        st["api_filled_by_auto"] += 1
                    elif any(k in header_map for k in keys):
                        st["api_filled_by_header_override"] += 1
                    else:
                        st["api_filled_by_fulltext_vote"] += 1
