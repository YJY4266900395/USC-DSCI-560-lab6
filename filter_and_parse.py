...
FIG1_PRIOR = rx_list([r"\bWELL\s*DATA\s*SUMMARY\b", r"\bSURFACE\s*LOCATION\b", r"\bBOTTOM\s*HOLE\s*LOCATION\b",
                      r"\bWell\s*Name\s*and\s*Number\b", r"\bWell\s*File\s*No\b", r"\bNDIC\s*File\s*Number\b"])

FIG2_STRONG = rx_list([r"\bWELL\s*SPECIFIC\s*STIMULATIONS\b", r"\bDATE\s*STIMULATED\b", r"\bACID,\s*SHOT,\s*FRAC\b", r"\bSTIMULATION\s+FORMATION\b"])
...
FIG2_PRIOR = rx_list([r"\bWELL\s*SPECIFIC\s*STIMULATIONS\b", r"\bDATE\s*STIMULATED\b", r"\bACID,\s*SHOT,\s*FRAC\b",
                      r"\bPROPPANT\b", r"\bSTAGES\b", r"\bTOP\b", r"\bBOTTOM\b"])
...
RE_DATUM_INLINE = re.compile(r"\bDatum\s*[:#]?\s*([A-Za-z0-9][A-Za-z0-9 \-_/]{1,40})\b", re.I)
RE_SHL_INLINE   = re.compile(r"\b(?:Well\s+Surface\s+Hole\s+Location\s*\(SHL\)|Surface\s+Location)\s*[:#]?\s*([^\n\r]{1,180})", re.I)
...
def extract_datum(joined: str):
    v = first(RE_DATUM_INLINE, joined) or find_value_after_label(joined, "Datum")
    return None if (v and BAD_VALUE_RX.search(v)) else clean_line(v)

def extract_shl(joined: str):
    v = (first(RE_SHL_INLINE, joined)
         or find_value_after_label(joined, "Well Surface Hole Location (SHL)")
         or find_value_after_label(joined, "Surface Location"))
    return None if (v and BAD_VALUE_RX.search(v)) else clean_line(v)
...
def bbox_for_api(api10):
    return (45.5, 49.5, -105.5, -96.0) if api10 and str(api10).startswith("33-") else (24.0, 50.0, -125.0, -66.0)
...
def parse_lat_lon(text: str, api10):
    bbox = bbox_for_api(api10)
...
def extract_latlon_from_pages(pages, api10, max_scan=300):
    ...
    for p in uniq[:max_scan]:
        lat, lon, lat_raw, lon_raw, suspect = parse_lat_lon(p.get("text") or "", api10)
...
def parse_stim(fig2_pages):
    ...
    has_fields = any([date_iso, formation, top, bottom, stages, vol, units])
    stim_present = bool(strong or has_fields or any([treatment_type, acid_pct, lbs_proppant, max_treatment_pressure_psi, max_treatment_rate_bbl_min, details]))
...
def labeliness_fig1(t: str) -> int:
    kws = ["Well Name and Number","Well Name","Operator","Address","County, State",
...
def parse_well(fig1_pages, all_pages, rel_path: str, latlon_scan_pages: int):
    ...
    well_name = pick_label_value(joined, ["Well Name and Number", "Well Name"])
    op = pick_label_value(joined, ["Operator"])
    addr = pick_label_value(joined, ["Address"])

    if op:
        op2 = re.sub(r"\(?\d{3}\)?[-\s]?\d{3}[-\s]?\d{4}.*$", "", op).strip()
        op = op2 or op

    county_state = first(RE_COUNTY_STATE, joined) or find_value_after_label(joined, "County, State")
    if county_state and BAD_VALUE_RX.search(county_state):
        county_state = first(RE_COUNTY_STATE, joined)

    county, state = split_county_state(county_state)
    datum = extract_datum(joined)
    shl = extract_shl(joined)

    lat, lon, lat_raw, lon_raw, suspect = parse_lat_lon(joined, api10)
    latlon_page = None
    if (lat is None or lon is None) and all_pages:
        lat, lon, lat_raw, lon_raw, pno, suspect2 = extract_latlon_from_pages(
            all_pages, api10, max_scan=latlon_scan_pages
        )
...
def main():
    ...
    for jf in text_files:
        try:
            ...
            stim = parse_stim(fig2_pages) if fig2_pages else dict(stim_present=False, stim_has_fields=False, raw_text="", fig2_pages=[])

            primary_id = well.get("api") or (f"NDIC-{well.get('ndic_file_no')}" if well.get("ndic_file_no") else rel_path)

            well_row = dict(
                primary_id=primary_id,
                source_pdf=src_pdf, relative_path=rel_path,
                api=well.get("api"),
                ...
                well_surface_location=well.get("well_surface_location"),
            ...
            if well_row.get("well_surface_location"): st["shl_present"] += 1
            ...
            if len(wells_buf) >= 200: append_jsonl(wells_buf, well_out); wells_buf.clear()
            if len(stim_buf)  >= 200: append_jsonl(stim_buf,  stim_out); stim_buf.clear()
